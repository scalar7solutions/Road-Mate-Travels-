...
export default defineComponent({
  ...
  methods: {
    search() {
      router.get(route('destinationgroups.index'), { q: this.q }, { preserveState: true, replace: true })
    },
    destroyItem(id: number) {
      if (!confirm('Delete this destination group? Places under it will also be deleted.')) return
      router.delete(route('destinationgroups.destroy', id), { preserveScroll: true })
    },
  },
})
</script>
...
<template>
  <AppLayout>
    <Head title="Destination Groups" />

    <div class="p-6 space-y-4">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 class="text-2xl font-bold">Destination Groups</h1>
          <p class="text-sm text-neutral-500">Manage destination headers (e.g., Down South, Nuwara Eliya).</p>
        </div>

        <Link
          :href="route('destinationgroups.create')"
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="r in (items as any).data" :key="r.id" class="border-t">
              ...
              <td class="p-3 text-neutral-600">{{ r.slug }}</td>
              <td class="p-3">{{ r.sort_order }}</td>
              <td class="p-3">
                <span
              ...
              <td class="p-3 text-right">
                <Link
                  :href="route('destinationgroups.edit', r.id)"
